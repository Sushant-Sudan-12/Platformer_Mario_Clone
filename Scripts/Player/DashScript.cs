...
public class DashScript : MonoBehaviour
{
    ...
    private void ProcessDash()
    {
        ...
        // Check if the player is on the ground before allowing the dash
        if (Input.GetButtonDown("Fire1") && isDash)
        {
    ...
    private IEnumerator DashCoroutine(float horizontalInput, float verticalInput)
    {
        ...
        if (GetComponentInParent<Movement>().OnGround)
        {
            // If on the ground, use the input direction as-is
            dashDirection = new Vector2(horizontalInput, verticalInput).normalized;
        }
        else
        {
            // If in the air, limit the vertical component of the dash
            dashDirection = new Vector2(horizontalInput, Mathf.Clamp(verticalInput, 0, 1)).normalized;
        }
